...
#클라이언트(앱)에서 보낸 사진을 받아서버에 저장하는 함수
#사용자아이디(userId)를 받아와 폴더를 생성 후 하위에
#original폴더 생성 후  비교할 사진들 받고
#source폴더 생성 후 소스사진들 저장하는 걸로 바꿀 것
@app.route('/', methods = ['GET', 'POST'])
def handle_request():
    files_ids = list(flask.request.files) #받아온 사진들 리스트
...
app.run(host="0.0.0.0", port=5000, debug=True) #0.0.0.0으로 해야 모든 곳에서 접근 가능
...
#얼굴 비교 함수
def compare_faces(path, userId, name, sourceFile, targetFile):
...
#이 아래 코드들을 handle_requset로 옮겨야 할 듯
def main():
    userId = 'blackpink' # !!이것도 받아와야 함, 추후 변경할 것
    path = os.getcwd()  #python 설치 경로, 서버도 마찬가지로//
    ...
    shutil.rmtree(path+'/blackpink/original')
